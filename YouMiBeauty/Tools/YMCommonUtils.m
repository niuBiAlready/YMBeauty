...
#import "YMCommonUtils.h"

@implementation YMCommonUtils
...
+(NSDictionary*)securityMethod:(NSDictionary *)dic isSecurity:(BOOL)security
{
    ...
    else
    {
        ...
        NSString * securityString = [str enSecurityWithKey:kSecurityXiaoMiKey];
        NSDictionary *tempDic =[NSDictionary dictionaryWithObjectsAndKeys:securityString,@"safejson", nil];
        return tempDic;
    }
}
...
+(NSMutableAttributedString *)parseSpanString:(NSString*)string withSpanStartStr:(NSString*)spanStart spanEndStr:(NSString*)spanEnd spanColor:(UIColor*)color
{
    ...
    message= [message stringByReplacingOccurrencesOfString:spanEnd withString:spanStart];
    
    NSMutableArray * tArray = [[NSMutableArray alloc] init];
    NSRange range = [message rangeOfString:spanStart];
    if (range.location!=NSNotFound) {
        NSInteger index = 0;
        [tArray addObject:NSStringFromRange(range)];
        message = [message stringByReplacingCharactersInRange:range withString:@""];
        while (range.location !=NSNotFound) {
            range = [message rangeOfString:spanStart];
            if (range.location != NSNotFound) {
                index++;
                message = [message stringByReplacingCharactersInRange:range withString:@""];
                [tArray addObject:NSStringFromRange(range)];
    ...
    NSString *strNosString = [message stringByReplacingOccurrencesOfString:spanStart withString:@""];
    strNosString = [strNosString stringByReplacingOccurrencesOfString:spanEnd withString:@""];
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:strNosString];
    
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setLineSpacing:5.0f];//调整行间距
    [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [strNosString length])];
    
    if(0 == tArray.count%2)
    {
        for (int i =0; i < tArray.count;i=i+2) {
            if (0==i%2) {
                NSString * string0 = [tArray objectAtIndex:i];
                NSString * string1 = [tArray objectAtIndex:i+1];
                NSRange range0 = NSRangeFromString(string0);
                NSRange range1 = NSRangeFromString(string1);
                [attributedString addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(range0.location, range1.location-range0.location)];
            }
...
+(BOOL)checkIsNotFirstLaunch
{
    ...
    NSString * oldVersion = [userDefaults objectForKey:kZCNUserDefaultsKeyNotFirstLaunch];
    if ([kCurrentVersion isEqualToString:oldVersion]) {
        return YES;
    }
    return NO;
}
